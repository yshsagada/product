...
@DisplayName("카테고리 최저 상품 가격 변경 통합 테스트")
class CategoryLowestPriceUpdateStrategyTest extends IntegrationTestSupport {

    @Autowired
    CategoryLowestPriceUpdateStrategy strategy;
    ...
    void createCategoryLowestPrice()
    {
        ...
        String productId = range.stream().findFirst().orElseThrow();
        Double price = redisTemplate.opsForZSet().score(RedisKeyGenerator.Category(CategoryType.BAG), productId);
        assertThat(price.longValue()).isEqualTo(50);
    }
}